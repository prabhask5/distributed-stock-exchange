...
public:
    ...
    {
        m_publisher = PublisherPtr(m_participant->create_publisher(eprosima::fastdds::dds::PUBLISHER_QOS_DEFAULT, nullptr));
        return !(m_publisher == nullptr);
    }

    bool create_subscriber()
    {
        m_subscriber = SubscriberPtr(m_participant->create_subscriber(eprosima::fastdds::dds::SUBSCRIBER_QOS_DEFAULT, nullptr));
    ...
    TopicTuplePtr<TOPIC_TYPE> make_topic(const std::string &topic_name)
    {
        ...
        std::unique_ptr<TypeSupport> type_support_ptr = std::make_unique<TypeSupport>(std::make_unique<TOPIC_SUPPORT_TYPE>());

        type_support_ptr->register_type(m_participant.get());

        std::unique_ptr<Topic> topic_ptr(m_participant->create_topic(topic_name, type_support_ptr->get_type_name(), eprosima::fastdds::dds::TOPIC_QOS_DEFAULT));

        return std::make_unique<TopicTuple<TOPIC_TYPE>>(std::move(topic_ptr), std::move(topic_type_ptr), std::move(type_support_ptr));
    }
                                                          ...
                                                          const std::vector<std::string> &expression_parameters, bool quote_all_expressions = true)
    {
        ...
        return std::make_unique<DataReaderTuple<TOPIC_TYPE>>(std::move(filter_ptr), std::move(drl_ptr), std::move(data_reader));
    };
                                                          ...
                                                          eprosima::fastdds::dds::DataReaderListener *data_reader_listener)
    {
        ...
        return std::make_unique<DataReaderTuple<TOPIC_TYPE>>(nullptr, std::move(drl_ptr), std::move(data_reader));
    };
    ...
    DataWriterPtr make_data_writer(const TopicTuplePtr<TOPIC_TYPE> &topic_tuple)
    {
        ...
        return DataWriterPtr(m_publisher->create_datawriter(topic, eprosima::fastdds::dds::DATAWRITER_QOS_DEFAULT, std::make_unique<PubListener>(topic)));
    }
    ...
    {
        auto topic = std::get<0>(*topic_tuple).get();
        return DataWriterPtr(m_publisher->create_datawriter(topic, eprosima::fastdds::dds::DATAWRITER_QOS_DEFAULT, listener));
    }
