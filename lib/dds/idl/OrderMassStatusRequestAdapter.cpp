...
void OrderMassStatusRequestAdapter::FIX2DDS(
  ...
  if (fixMsg.isSetField(FIX::FIELD::MassStatusReqID))
    ddsMsg.MassStatusReqID(
        ((FIX::MassStatusReqID)fixMsg.getField(FIX::FIELD::MassStatusReqID))
            .getString()
            .c_str());
...
void OrderMassStatusRequestAdapter::DDS2FIX(
    const DistributedStockExchange_OrderMassStatusRequest::
        OrderMassStatusRequest &ddsMsg,
    FIX::Message &fixMsg) {
  HeaderAdapter::DDS2FIX(ddsMsg.fix_header(), fixMsg.getHeader());

  convert_dds_string_to_fix(ddsMsg.MassStatusReqID(),
                            FIX::FIELD::MassStatusReqID, fixMsg);
