...
void BusinessMessageRejectAdapter::FIX2DDS(const FIX::Message& fixMsg, DistributedStockExchange_BusinessMessageReject::BusinessMessageReject& ddsMsg )
{
...
void BusinessMessageRejectAdapter::DDS2FIX(const DistributedStockExchange_BusinessMessageReject::BusinessMessageReject& ddsMsg, FIX::Message& fixMsg)
{
	HeaderAdapter::DDS2FIX(ddsMsg.fix_header(), fixMsg.getHeader());

	convert_dds_string_to_fix(ddsMsg.RefMsgType(), FIX::FIELD::RefMsgType, fixMsg);
