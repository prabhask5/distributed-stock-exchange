...
void SecurityListAdapter::FIX2DDS(const FIX::Message& fixMsg, DistributedStockExchange_SecurityList::SecurityList& ddsMsg )
{
...
void SecurityListAdapter::DDS2FIX(const DistributedStockExchange_SecurityList::SecurityList& ddsMsg, FIX::Message& fixMsg)
{
	...
	convert_dds_string_to_fix(ddsMsg.SecurityResponseID(), FIX::FIELD::SecurityResponseID, fixMsg);
	...
	for ( long NoRelatedSym_group_cnt_index = 0; NoRelatedSym_group_cnt_index<ddsMsg.c_NoRelatedSym().size();NoRelatedSym_group_cnt_index++)
	{
		FIX::Group grp(FIX::FIELD::NoRelatedSym,FIX::FIELD::Symbol,FIX::message_order(FIX::FIELD::Currency,FIX::FIELD::RoundLot,FIX::FIELD::MinTradeVol,FIX::FIELD::TradingSessionID,FIX::FIELD::TradingSessionSubID,FIX::FIELD::ExpirationCycle,FIX::FIELD::Text,FIX::FIELD::EncodedTextLen,FIX::FIELD::EncodedText,0));
;
	convert_dds_string_to_fix(ddsMsg.c_NoRelatedSym()[NoRelatedSym_group_cnt_index].Symbol(), FIX::FIELD::Symbol, grp);

	convert_dds_string_to_fix(ddsMsg.c_NoRelatedSym()[NoRelatedSym_group_cnt_index].SecurityExchange(), FIX::FIELD::SecurityExchange, grp);

	convert_dds_string_to_fix(ddsMsg.c_NoRelatedSym()[NoRelatedSym_group_cnt_index].Text(), FIX::FIELD::Text, grp);
