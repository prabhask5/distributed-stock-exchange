...
void HeaderAdapter::FIX2DDS(const FIX::FieldMap& fixMsg, DistributedStockExchange::Header& ddsMsg )
{
...
void HeaderAdapter::DDS2FIX( const DistributedStockExchange::Header& ddsMsg, FIX::FieldMap& fixMsg)
{
	convert_dds_string_to_fix(ddsMsg.BeginString(), FIX::FIELD::BeginString, fixMsg);
	...
	convert_dds_string_to_fix(ddsMsg.SenderCompID(), FIX::FIELD::SenderCompID, fixMsg);

	convert_dds_string_to_fix(ddsMsg.TargetCompID(), FIX::FIELD::TargetCompID, fixMsg);

	FIX::MsgSeqNum fixMsgSeqNum(ddsMsg.MsgSeqNum());
	fixMsg.setField(fixMsgSeqNum);

	convert_dds_timestamp_to_fix(ddsMsg.SendingTime(), FIX::FIELD::SendingTime, fixMsg);
