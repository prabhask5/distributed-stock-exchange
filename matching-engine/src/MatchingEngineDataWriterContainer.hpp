...
struct DataWriterContainer {
  ...
  // through the FIX gatway. This is basically the response to the order cancel
  // FIX request. This DW goes from matching engine -> FIX gateway.
  ...
  // when the last refresh was sent to now. These market data refreshes are
  // periodically sent via a constant period duration
  // (marketdata.data_pub_interval in config file). This DW goes from matching
  // engine -> data service.
  DataWriterPtr marketDataIncrementalRefreshDW;

  // Represents information related to the list of securities contained within
  // this matching engine. This is basically the response to a security list FIX
  // request send by a user. This DW goes from matching engine -> FIX gateway.
  DataWriterPtr securityListRequestDW;

  // Represents information related to the market data related to this matching
  // engine. Market data can contain a list of securities, prices, list of
  // orders, etc. This is basically the response to the market data FIX request.
