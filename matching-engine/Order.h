...
public:
    ...
    Order(const std::string &clientId, const std::string &symbol,
          ...
          Side side, Type type, double price, double stopPrice, long quantity)
        : m_clientId(clientId), m_symbol(symbol), m_owner(owner), m_target(target), m_side(side), m_type(type), m_price(price), m_stopPrice(stopPrice),
          m_quantity(quantity)
    {
    ...
    double getPrice() const { return m_price; }         // 0 for market orders
    double getStopPrice() const { return m_stopPrice; } // 0 for non limit or stop orders
