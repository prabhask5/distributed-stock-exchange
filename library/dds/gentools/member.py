class Member:
    ...
    def get_logger_statement(self, entity_name):
        return (
            " << "
            '"ddsMsg.' + self.name + ' : " << ddsMsg.' + self.name + "() << std::endl"
        )
    ...
    def get_cpp_member_statement(self, member_name):
        ...
        fix2dds_cpp_def += (
            "\tif (" + self.fixObj + ".isSetField(FIX::FIELD::" + member_name + ") )\n"
        )
        ...
        if (
            self.type.attrib["dds"] == "float"
            or self.type.attrib["dds"] == "long"
            or self.type.attrib["dds"] == "unsigned long long"
        ):
    ...
    def get_cpp_declare_statement(self):
        ...
        if self.gen_requirements is None or self.gen_requirements.get("gen_cpp") == "Y":
            fix2dds_member_statement_cpp, dds2fix_member_statement_cpp = (
                self.get_cpp_member_statement(self.name)
            )
...
class GroupMember(Member):
    ...
    def get_cpp_declare_statement(self):
        ...
        fix2dds_cpp_statement, dds2fix_cpp_statement = self.get_cpp_dds_to_fix(
            group_count_index_var
        )
        ...
        fix2dds_cpp_def += (
            "\t\t" + self.get_fix_group_definition(self.group_components) + ";\n"
        )
        fix2dds_cpp_def += (
            "\t\tfixMsg.getGroup(" + group_count_index_var + "+1, grp);\n"
        )
        ...
        for group_component in self.group_components:
            if group_component.tag == "field":
                if not self.component_parser.is_required(group_component):
                    ...
                    if (
                        self.name
                        not in self.component_parser.requiredFieldsComponents.required_fields
                    ):
                        continue

                    required_fields = self.component_parser.requiredFieldsComponents.components_required_fields_components[
                        self.name
                    ]
                ...
                if (
                    field_type
                    in self.component_parser.requiredFieldsComponents.types.keys()
                ):  # and self.name in  self.component_parser.requiredFieldsComponents.required_components.keys():
                    type = self.component_parser.requiredFieldsComponents.types[
                        field_type
                    ]
                    ...
                    member = Member(
                        ...
                        field_name,
                        "grp",
                        "ddsMsg.c_" + self.name + "()[" + index_var + "]",
                        gen_requiements,
                    )
                    fix2dds_cpp_statement, dds2fix_cpp_statement = (
                        member.get_cpp_declare_statement()
                    )
                    fix2dds_cpp_def += fix2dds_cpp_statement
                    dds2fix_cpp_def += dds2fix_cpp_statement
                ...
                gen_requiements = None
                if (
                    sub_component_name in self.component_parser.compenent_dict.keys()
                    and sub_component_name
                    in self.component_parser.requiredFieldsComponents.required_components.keys()
                ):
                    sub_components = self.component_parser.compenent_dict[
                        sub_component_name
                    ]
                ...
                print(
                    "TODO: Add nested groups handler - ["
                    + group_component.tag
                    + "-"
                    + group_component.get("name")
                    + "] - "
                    + self.name
                )
    ...
    def get_fix_field(self, group_component):
        ...
        if group_component.tag == "component":
            ...
            if group_component.get("name") == "Instrument":
                ...
                return "GenSrc - Investigate"
                # To Fix, identify required fields in a given component, select tag of the first field

        elif group_component.tag == "field":
            return "FIX::FIELD::" + group_component.get("name")
        else:
            print(
                "Undefine group element : "
                + group_component.get("name")
                + " : "
                + group_component.tag
            )
    ...
    def get_fix_group_definition(self, group_components):
        ...
        group_definition = (
            "FIX::Group grp(FIX::FIELD::"
            + self.type
            + ","
            + self.get_fix_field(group_components[0])
            + ",FIX::message_order("
        )
