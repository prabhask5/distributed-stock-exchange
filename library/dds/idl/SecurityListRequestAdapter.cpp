...
void SecurityListRequestAdapter::FIX2DDS(
  ...
  if (fixMsg.isSetField(FIX::FIELD::SecurityListRequestType))
    ddsMsg.SecurityListRequestType(
        FIELD_GET_REF(fixMsg, SecurityListRequestType));
  else
...
void SecurityListRequestAdapter::DDS2FIX(
  ...
  FIX::SecurityListRequestType fixSecurityListRequestType(
      ddsMsg.SecurityListRequestType());
